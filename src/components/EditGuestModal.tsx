      ...
      {isOpen && guest && (
        ...
        <div className="flex items-center justify-between pb-6 border-b gold-border mb-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full cream-bg text-[#2D2D2D] flex items-center justify-center border gold-border">
              <Edit3 className="w-5 h-5 gold-text" />
            </div>
            <div>
              <h2 className="text-2xl serif font-bold text-[#2D2D2D]">Edit Guest</h2>
              <p className="text-[10px] uppercase tracking-widest opacity-60 text-[#2D2D2D]">Update name or family side</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="edit-guest-name-input" className="block text-xs uppercase tracking-widest font-bold text-[#2D2D2D] mb-2">
              Guest Name <span className="text-rose-500">*</span>
            </label>
            <input
              ref={inputRef}
              id="edit-guest-name-input"
              type="text"
              value={name}
            ...
            <div className="grid grid-cols-2 gap-4">
              <motion.button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all cursor-pointer ${
                  side === 'groom'
                    ? 'sage-bg border-[#2D2D2D] shadow-xs'
              ...
              <motion.button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all cursor-pointer ${
                  side === 'bride'
                    ? 'cream-bg-alt border-[#2D2D2D] shadow-xs'
                    : 'bg-[#F9F6F0] border-transparent hover:border-gray-300 opacity-70'
                }`}
              >
